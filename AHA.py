...
"""
@author: justin
/media/justin/justin/neutron_water_orientation/protein/6eym/set_d/from_org_refined_no_wat
--pdb=6eym_del_D.pdb --out=adH.pdb --map=6eym_no_prot_D.ccp4  --pKa=no --wat=no --not_del=140 --EDIA=no


/media/justin/justin/neutron_water_orientation/protein/5ce4
--pdb=5ce4_noH_prot.pdb --out=adH.pdb --map=5ce4_2mFo-DFc_map.ccp4

/media/justin/justin/Acetylcholinesterase/2y2v_sort/2y2v_sort/P03
--pdb= P03.pdb --out=adH.pdb --map=P03_2mFo-DFc_map.ccp4 --pKa=no --wat=no --EDIA=yes
...
mesh = 0.10
#defines if H or D labels are used
label_H="D"
##input variables:
    
parser=argparse.ArgumentParser()
parser.add_argument('--pdb', help='input pdb file',required=True)
parser.add_argument('--map', help='input ccp4 map',required=True)
parser.add_argument('--out', help='output file')
parser.add_argument('--wat', help='protonate water, yes or no')
parser.add_argument('--not_del',nargs='*', help='residues where hydrogends do not get delete')
parser.add_argument('--pKa', help='set jugment of hydrogens in hydrogen bond based on teh pKa value and not on the densety')
parser.add_argument('--EDIA',help='use EDIA yes or no')
parser.add_argument('--rad', help='rad file')
parser.add_argument('--rx', help='factor for integration radius')
...
if args.wat != None:
           wat=args.wat
           if wat=="yes" or wat=="y" or wat=="YES" or wat=="Y" or wat=="True" :
                wat_ex=True
...
if args.not_del != None:
           not_del_l=args.not_del
           for i in range(len(not_del_l)):
               not_del_l[i]=int(not_del_l[i])
           print("rsidues whre protons are not getting deletet ",not_del_l)
else:
           not_del_l=[]
           print("all hydgrogens are kept ")
if args.pKa != None:
           pka=args.pKa
           if pka=="yes" or pka=="y" or pka=="YES" or pka=="Y" or pka=="True" :
                pka_ex=True
                print("pKa values are used")
           else:
              pka_ex=False
              print("pKa values are not used")
else:
              pka_ex=False
              print("pKa values are not used")
...
if args.EDIA != None:
           EI=args.EDIA
           if EI=="yes" or EI=="y" or EI=="YES" or EI=="Y" or EI=="True" :
                EDIA_ex=True
                print("EDIA is used")
           else:
              EDIA_ex=False
              print("EDIA is not used")
else:
              EDIA_ex=False
              print("EDIA is not used")    
...
if args.rad != None:
   input_rad=args.rad
   print("input rad file is:", input_rad)
else:
         input_rad="/media/justin/justin/scripts/rad.csv"
         print("input rad file is:", input_rad)
              
if args.rx != None:
           rx=float(args.rx)
           print("factor for the integration rx ",rx)
else:
           rx=1
           print("factor for the integration rx ",rx)
...
#read ccp4 map with gemmi
        ...
        print(gemmi.InfoMap())
        map_arr=np.array(map.grid,copy=False)
#        x = np.linspace(0, map.grid.unit_cell.a, num=map_arr.shape[0], endpoint=False)
#        y = np.linspace(0, map.grid.unit_cell.b, num=map_arr.shape[1], endpoint=False)
#        z = np.linspace(0, map.grid.unit_cell.c, num=map_arr.shape[2], endpoint=False)
        print("x,y,z",map.grid.unit_cell.a,map.grid.unit_cell.b,map.grid.unit_cell.c)
        print("STD",np.std(map_arr))
        map_std=np.std(map_arr)
        map_ave=np.average(map_arr)
        print("mean",np.mean(map_arr))
        print("average",np.average(map_arr))
        print("head",map.header_str)
        print(map.__repr__())
#       print(help(map))
        print(" map is readet")
        return map, map_std, map_ave
...
#         print("res",pdb[res_num][0][6])
          ...
          if pdb[res_num][0][4]=="WAT" or pdb[res_num][0][4]=="HOH" or pdb[res_num][0][4]=="DOD":
...
#           for i in range(len(pdb[res_num])):
#               print("water atom",i,pdb[res_num][i])
...
#                print("¤¤¤¤¤¤¤¤¤¤¤¤¤¤1")
#                print(pdb[res_num][0])
                com = ceck_bond_atoms_N(pdb,res_num,res_num-1,alter[alt],"N","CA", "C")
#                print("com",com)
#                print("¤¤¤¤¤¤¤¤¤¤¤¤¤¤2")
#               if len(alter)>1:
...
## givs covalend radius for elment
        ...
        return r
def pks_res():
    #side chain pks values of amino acides
    pks=[["CYS",8.3],["SER",15,13],["THR",15,13],["TYR",15,10.1],["ASP",3.9],["GLU",4.3],["HIS",6.0],["ARG",12.5],["LYS",10.5]]
    pks.append(["HOH",14])
    pks.append(["DOD",14])
    pks.append(["WAT",14])
...
def atom_to_pks(atom):
    # defines teh pks of sidechain an atom is in
    l_pks=pks_res()
    pks=False
...
def rm_cov_clash(pdb,clash_cov,map):
## remove atom involved in covalent clash with lower integratet densety from pdb
    print("remove covalend clashes (dleet lower integrattet hydrigen) ")
    for i in range(len(clash_cov)):
        for j in range(len(clash_cov[i])):
            for k in range(len(pdb)):
                if clash_cov[i][j][5]==pdb[k][0][5] and clash_cov[i][j][6]==pdb[k][0][6]:
                    for l in range(len(pdb[k])):
                        if clash_cov[i][j][2]==pdb[k][l][2] and clash_cov[i][j][3]==pdb[k][l][3]:
#                            print("clash",clash_cov[i][j])
#                            print("pdb  ",pdb[k][l])
                            clash_cov[i][j].append([k,l])
#                            if clash_cov[i][j][14]=="D" or clash_cov[i][j][14]=="H":
#                               coord=[clash_cov[i][j][8],clash_cov[i][j][9],clash_cov[i][j][10]]
#                               int_D=int_around_point(coord,map,def_r_int(clash_cov[i][j][14]),mesh)
#                               clash_cov[i][j].append(int_D)
    for i in range(len(clash_cov)):
        dif_int= clash_cov[i][0][16]-clash_cov[i][1][16]                      
        if dif_int>0:
            del pdb[clash_cov[i][1][17][0]][clash_cov[i][1][17][1]]
            print("Delet atom  ",clash_cov[i][1][2],clash_cov[i][1][3],clash_cov[i][1][4],clash_cov[i][1][6],clash_cov[i][1][5],clash_cov[i][1][1])
        if dif_int<0:
            del pdb[clash_cov[i][0][17][0]][clash_cov[i][0][17][1]]
            print("Delet atom ",clash_cov[i][0][2],clash_cov[i][0][3],clash_cov[i][0][4],clash_cov[i][0][6],clash_cov[i][0][5],clash_cov[i][0][1])
...
def del_d_hbonds(pdb):
# delets weaker H if two are presend in one hydrogen bond
    d_line=0.2
    l_del_atom=[]
    for i in range(len(pdb)):
        for k in range(len(pdb[i])):
            if pdb[i][k][14]=="O" or pdb[i][k][14]=="N":
                xyz1=[pdb[i][k][8],pdb[i][k][9],pdb[i][k][10]]
                vdw1=ele_to_vdw_rad(pdb[i][k][14])
                for l in range(len(pdb)):
                    for m in range(len(pdb[l])):
                      xyz2=[pdb[l][m][8],pdb[l][m][9],pdb[l][m][10]]
                      vdw2=ele_to_vdw_rad(pdb[l][m][14])
                      d = CDist2(xyz1,xyz2)
                      sum_vdw=vdw1+vdw2
                      if d <= sum_vdw:
##                         print("alter",pdb[i][k][3],pdb[l][m][3])
#                         if pdb[i][k][3]==pdb[l][m][3] or pdb[l][m][3]==" " or pdb[i][k][3] == " ":
#                            alter_prot=[pdb[l][m][3],pdb[i][k][3]]
##                            print("alter_prot",alter_prot)
                            d_bond_H=[]
                            for n in range(len(pdb[i])):
                                if pdb[i][n][14]=="D" or pdb[i][n][14]=="H":
#                                 if pdb[i][n][3] == alter_prot[0] or pdb[i][n][3] == alter_prot[1]:
                                    xyz3=[pdb[i][n][8],pdb[i][n][9],pdb[i][n][10]]
                                    dist_to_hbond=d_p_to_line(xyz3,xyz2,xyz1)
                                    d1= CDist2(xyz3,xyz1)
                                    d2= CDist2(xyz3,xyz2)
                                    if d1 <=d and d2<=d:
                                      if dist_to_hbond<= d_line:
                                        d_bond_H.append([pdb[i][n],i,n])
                            for n in range(len(pdb[l])):
                                if pdb[l][n][14]=="D" or pdb[l][n][14]=="H":
 #                                 if pdb[l][n][3] == alter_prot[0] or pdb[l][n][3] == alter_prot[1]:
                                    xyz3=[pdb[l][n][8],pdb[l][n][9],pdb[l][n][10]]
                                    dist_to_hbond=d_p_to_line(xyz3,xyz2,xyz1)
                                    d1= CDist2(xyz3,xyz1)
                                    d2= CDist2(xyz3,xyz2)
                                    if d1 <=d and d2<=d:
                                      if dist_to_hbond<= d_line:
                                        d_bond_H.append([pdb[l][n],l,n])
                            if len(d_bond_H)>1:
#                                print("number of atoms in H bond",len(d_bond_H))
                                del_entry=[]
                                for o in range(len(d_bond_H)-1):
                                    
                                    for h in range(o+1,len(d_bond_H)):
#                                        print("h",h,"o",o,d_bond_H[o][2],d_bond_H[h][2])
                                        if d_bond_H[o][2]==d_bond_H[h][2]:
                                            print("#######################")
                                            print("del alter:",d_bond_H[o][0][3],d_bond_H[h][0][3])
                                            print("atom 1",d_bond_H[h][0])
                                            print("atom 2",d_bond_H[o][0])
                                            del_entry.append(h)
#                                print("del_entry",del_entry)
                                for p in range(len(del_entry),0,-1):
#                                    print(p)
                                    del d_bond_H[p]
#                                print("len after clear",len(d_bond_H))
                                if len(d_bond_H)>1:
                                  print("number of atoms in H bond",len(d_bond_H))
                                  int_atom=[100,0]
                                  for q in range(len(d_bond_H)):
                                      if len(d_bond_H[q])<=15:
                                          xyz=[d_bond_H[q][0][8],d_bond_H[q][0][9],d_bond_H[q][0][10]]
                                          int_H=int_around_point(xyz,map,def_r_int("H"),mesh)
                                          if int_atom[0] > int_H:
                                              int_atom[0]=int_H
                                              int_atom[1]=q
                                      if len(d_bond_H[q])>=16:
                                          int_H=d_bond_H[q][16]
                                          if int_atom[0] > int_H:
                                              int_atom[0]=int_H
                                              int_atom[1]=q
                                  print("atom to tel",d_bond_H[int_atom[1]][1],d_bond_H[int_atom[1]][2])
                                  l_del_atom.append([d_bond_H[int_atom[1]][1],d_bond_H[int_atom[1]][2]])
                                  
#                                del pdb[d_bond_H[int_atom[1]][1]][d_bond_H[int_atom[1]][2]]
    l_del_atom_pure=[]
    for sublist in l_del_atom:
        if sublist not in l_del_atom_pure:
            l_del_atom_pure.append(sublist)
    print(((l_del_atom_pure)))     
    for i in range(len(l_del_atom_pure)):
        print("del atom",pdb[l_del_atom_pure[i][0]][l_del_atom_pure[i][1]])
#        del pdb[l_del_atom_pure[i][0]][l_del_atom_pure[i][1]]                                 
                                          
#                                      print(d_bond_H[q])
                                    #print("atoms in conflict", d_bond_H[o])

    return pdb


def del_d_hbonds_new(pdb,not_del_l,pka_ex):
#delet weaker H if two hydrogens are in one bond
    #find potential hydgrogen bond
    print("Find potantial H bond")
    l_pot_H_bond=find_pot_H_bond(pdb)
    print(" number of potential H bond", len(l_pot_H_bond))
#    for i in range(len(l_pot_H_bond)):
#        print(l_pot_H_bond[i])
#    print(len(l_pot_H_bond))
    print("Find miss modled H")
    del_atoms,del_atoms_num=find_H_in_Hbond(l_pot_H_bond,pdb,pka_ex)
#    for i in range(len(del_atoms)):
#        print("del atom",i,del_atoms[i])
#        print("del atom number", del_atoms_num[i])
#        print(pdb[del_atoms[i][0]][del_atoms[i][1]])
    print("numbers of atoms to deleet",len(del_atoms))
    pdb = delet_atoms(del_atoms,pdb,not_del_l)
    print("wrongly placed hydrogends in H bonds are deleted")
                    
                     
                 
    return pdb

def delet_atoms(del_atoms,pdb,not_del_l):
    #delets atoms from del_atoms in pdb
    for i in range(len(del_atoms)):
        if del_atoms[i][2][6] in not_del_l:
           print("not delet atom",del_atoms[i])
           continue
        for j in range(len(pdb[del_atoms[i][0]])-1,0,-1):
#            print("j",j,len(pdb[del_atoms[i][0]]))
#            print("1",pdb[del_atoms[i][0]][j])
#            print("2",del_atoms[i][2][2])
            
            if pdb[del_atoms[i][0]][j][2]==del_atoms[i][2][2]:
               if pdb[del_atoms[i][0]][j][3]==del_atoms[i][2][3]:
                   print("delet atom",pdb[del_atoms[i][0]][j])
                   del pdb[del_atoms[i][0]][j]
    return pdb
        

def find_H_in_Hbond(pot_H,pdb,pka_ex):
    del_atoms=[]
    del_atoms_num=[]
#    H_in_Hbond=[]
    for i in range(len(pot_H)):
#      print("next bond")
      l_h_in_bond=[[],[]]
#      print(pot_H[i])
      atom1=pdb[pot_H[i][0][0]][pot_H[i][0][1]]
      atom2=pdb[pot_H[i][1][0]][pot_H[i][1][1]]
#      if atom1[4]=="HOH":
#          for a in range(len(pdb[pot_H[i][0][0]])):
#           print("HOH1",pdb[pot_H[i][0][0]][a])
          
#      if atom2[4]=="HOH":
#          for a in range(len(pdb[pot_H[i][1][0]])):
#           print("HOH2",pdb[pot_H[i][1][0]][a])
      
      res1=pdb[pot_H[i][0][0]]
      res2=pdb[pot_H[i][1][0]]
#      print("resi1",pot_H[i][0][0],"resi2",pot_H[i][1][0])
      xyz1=[atom1[8],atom1[9],atom1[10]]
      cov1=ele_to_cov_rad(atom1[14])
      xyz2=[atom2[8],atom2[9],atom2[10]]
      cov2=ele_to_cov_rad(atom2[14])
      d = CDist2(xyz1,xyz2)
      for j in range(len(res1)):
       
       if res1[j][14]=="H" or res1[j][14]=="D":
        if res1[j][3]==atom1[3]:
          
          xyzH=[res1[j][8],res1[j][9],res1[j][10]]
          covH=ele_to_cov_rad(res1[j][14])
          XH=CDist2(xyz1,xyzH)
          sum_cov=cov1+covH+0.4
          if XH <= sum_cov:
#             print("HOH1",res1[j]) 
             HA=CDist2(xyz2,xyzH)
             if HA < d:
                print("HOH11",res1[j])
#                print(res1[i])
#                print("pot_H[i]", pot_H[i])
                l_h_in_bond[0].append([res1[j],pot_H[i][0][0],j])
      for j in range(len(res2)):
       
       if res2[j][14]=="H" or res2[j][14]=="D":
        if res2[j][3]==atom2[3]: 
 #         print("HOH2",res2[j])
          xyzH=[res2[j][8],res2[j][9],res2[j][10]]
          covH=ele_to_cov_rad(res2[j][14])
          XH=CDist2(xyz2,xyzH)
          sum_cov=cov2+covH
          if XH <= sum_cov:
             HA=CDist2(xyz1,xyzH)
             if HA < d:
                print("HOH2",res2[j])
                l_h_in_bond[1].append([res2[j],pot_H[i][1][0],j])
#      print(l_h_in_bond)
#      print(len(l_h_in_bond[0]),len(l_h_in_bond[1]))
      if len(l_h_in_bond[0]) >=1 and len(l_h_in_bond[1])>=1:
#        print(l_h_in_bond)
        for e in range(len(l_h_in_bond[0])):
         for r in range(len(l_h_in_bond[1])):
          dH=d_atom(l_h_in_bond[0][e][0],l_h_in_bond[1][r][0])
          print("distance between H:" ,dH)
          print("l_h_in_bond[0][e][0]",l_h_in_bond[0][e][0])
          print("l_h_in_bond[1][r][0]",l_h_in_bond[1][r][0])
          if dH <= 2:
            print("distance between H:" ,dH)
            print("l_h_in_bond[0][e][0]",l_h_in_bond[0][e][0])
            print("l_h_in_bond[1][r][0]",l_h_in_bond[1][r][0])
            pks1=atom_to_pks(l_h_in_bond[0][e][0])
            pks2=atom_to_pks(l_h_in_bond[1][r][0])
            if pka_ex == True and pks1 != False and pks2 != False:
                print("pKa vlaue is used", pks1, pks2)
                if pks1 < pks2:
                    del_atoms.append([l_h_in_bond[0][e][1],l_h_in_bond[0][e][2],l_h_in_bond[0][e][0]])
                    print([l_h_in_bond[0][e][1],l_h_in_bond[0][e][2],l_h_in_bond[0][e][0]])
                    print([l_h_in_bond[1][r][1],l_h_in_bond[1][r][2],l_h_in_bond[1][r][0]])
                if pks1 >= pks2:
                    del_atoms.append([l_h_in_bond[1][r][1],l_h_in_bond[1][r][2],l_h_in_bond[1][r][0]])
                    print([l_h_in_bond[1][r][1],l_h_in_bond[1][r][2],l_h_in_bond[1][r][0]])
                    print([l_h_in_bond[0][e][1],l_h_in_bond[0][e][2],l_h_in_bond[0][e][0]])
            else:
                xyzH1=atom_to_coord(l_h_in_bond[0][e][0])
                int1=int_around_point(xyzH1,map,def_r_int("H"),mesh)

                xyzH2=atom_to_coord(l_h_in_bond[1][r][0])
                int2=int_around_point(xyzH2,map,def_r_int("H"),mesh)
#            print("H1",int1,"H2",int2)
                if int1 < int2:
#                   print("%%%%%",l_h_in_bond[0][e][1],l_h_in_bond[0][e][2],l_h_in_bond[0][e][0][1])
                    del_atoms.append([l_h_in_bond[0][e][1],l_h_in_bond[0][e][2],l_h_in_bond[0][e][0]])
                    del_atoms_num.append(l_h_in_bond[0][e][0][1])
                if int1 > int2:
#                print("¤¤¤¤¤",l_h_in_bond[1][r][1],l_h_in_bond[1][r][2],l_h_in_bond[1][r][0][1])
                    del_atoms.append([l_h_in_bond[1][r][1],l_h_in_bond[1][r][2],l_h_in_bond[1][r][0]])
                    del_atoms_num.append(l_h_in_bond[1][r][0][1])
    #        print(l_h_in_bond[0][e])
    #        print(l_h_in_bond[1][r])
            
             
  
    
    
    return del_atoms,del_atoms_num

    
def atom_to_coord(atom):
# extraxt xyz xoordinates of atom
    p=[atom[8],atom[9],atom[10]]
    return p

def d_atom(atom1,atom2):
#calculate distance betweenn PDB attoms
        A=[atom1[8],atom1[9],atom1[10]]
        B=[atom2[8],atom2[9],atom2[10]]
        dist = len3dvec(twoP_to_vec(A, B))
        return dist      
    

def find_pot_H_bond(pdb):
    l_pot_H_bond=[]
    for i in range(len(pdb)):
      for j in range(len(pdb[i])):
        if pdb[i][j][14]=="O" or pdb[i][j][14]=="N":
#            print(pdb[i][j])
            xyz1=[pdb[i][j][8],pdb[i][j][9],pdb[i][j][10]]
            vdw1=ele_to_vdw_rad(pdb[i][j][14])
            cov1=ele_to_cov_rad(pdb[i][j][14])
            for k in range(len(pdb)):
             for l in range(len(pdb[k])):
              if i != k and j !=l: 
                if pdb[i][j][3]=='' or pdb[k][l][3]=='' or pdb[i][j][3]==pdb[k][l][3]:
#                  if pdb[k][l][3]=='' :
#                   if pdb[i][j][6]==165:
#                    if pdb[k][l][6]==186:
#                     print(pdb[i][j])
#                     print("#####",pdb[k][l])
                 xyz2=[pdb[k][l][8],pdb[k][l][9],pdb[k][l][10]]
                 vdw2=ele_to_vdw_rad(pdb[k][l][14])
                 cov2=ele_to_cov_rad(pdb[k][l][14])
                 sum_vdw=vdw1+vdw2
                 sum_cov=cov1+cov2
                 d = CDist2(xyz1,xyz2)
                 if sum_cov <= d <= sum_vdw :
                    if i <= k:
                       l_pot_H_bond.append([[i,j],[k,l]])
                    if i > k:
                       l_pot_H_bond.append([[k,l],[i,j]])
                       
    l_potH=[]
    for sublist in l_pot_H_bond:
        if sublist not in l_potH:
            l_potH.append(sublist)    
    l_potH.sort()
    return l_potH



def read_rad(input_rad):
#read file with atom radi for EDIA
        print(" read rad file")
        read_rad = open(input_rad, "r")
        l_rad =[]
        for line in read_rad:
            sep=line.split("&")
            if sep[0]!="#":
                 l_rad.append(rad(sep))
            
        return l_rad

class rad:
# atoms contain all informations which are stores in the pdb file
    def __init__(self,line):   
        self.ele = line[0].strip()
        self.r05 = float(line[1].strip())
        self.r1  = float(line[2].strip())
        self.r15 = float(line[3].strip())
        self.r2  = float(line[4].strip())
        self.r25 = float(line[5].strip())
        self.r3  = float(line[6].strip())
        
def treat_res_EDIA(pdb,map,l_rad):
#work on each residio seperatly 
        alerts=[["CH"],["NH"],["OH"],["SH"],["flip"],["wat"],["disordert"],[]]
        d_res=find_d_res(head)
        print("the resolution of the structure is",d_res)
#       alerts =[CH,NH,OH,SH,flip]     
#       pdb, alerts = treat_n_nerm(pdb,map,alerts)
        for res_num in range(len(pdb)):
#         print("res",pdb[res_num][0][6])
...
#          if pdb[res_num][0][4]=="LYS":
#                   pdb, alerts = treat_LYS_EDIA( pdb, res_num, map, alerts )
          if pdb[res_num][0][4]=="SER":
                   pdb, alerts = treat_SER_EDIA( pdb, res_num ,map, alerts, d_res)
#          if pdb[res_num][0][4]=="THR":
#                   pdb, alerts = treat_THR( pdb, res_num ,map, alerts)
#          if pdb[res_num][0][4]=="TYR":
#                   pdb, alerts = treat_TYR( pdb, res_num ,map, alerts)
...
def clac_EDIA_CA(pdb,map,l_rad):
    alerts=[["CH"],["NH"],["OH"],["SH"],["flip"],["wat"],["disordert"],[]]
    d_res=find_d_res(head)
    for i in range(len(pdb)):
        for j in range(len(pdb[i])):
            if pdb[i][j][4]=="BGC":
#               print(pdb[i][j])
               EDIA_val=calc_EDIA(pdb[i][j],pdb[i][j][3],map,map_ave,map_std,l_rad,d_res)
               print("EDIA",pdb[i][j][4],pdb[i][j][2], round(EDIA_val,3))
              
    
        
def treat_LYS_EDIA(pdb,res_num, map, alerts):
# ad hydrogen to standard psoitions at LYS
        alter, alerts = find_alter(pdb,res_num,alerts)
        pdb = ad_NH(pdb,res_num,alter)
        pdb = ad_CH_R3(pdb, res_num,alter,"C","N","CA","CB")
...
def treat_SER_EDIA(pdb,res_num,map, alerts,d_res):
# ad hydrogen to standard psoitions at SER
        alter, alerts = find_alter(pdb,res_num,alerts)
        print("#######################")
        pdb, alerts = update_OH_EDIA(pdb, res_num,alter,"CA","CB","OG",map, alerts,d_res)
#        pdb, alerts = ad_OH_map(pdb, res_num,alter,"CA","CB","OG",map, alerts)
...
def update_OH_EDIA(pdb, res_num,alter, A1, A2, A3,map, alerts,d_res):
    for alt in range(len(alter)):
        ...
        if com == True:
                return pdb
        print("¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤")
        HO = find_bond_H(pdb,res_num,alter[alt],A3)
        print(HO)
        EDIA_val=calc_EDIA(HO,alter[alt],map,map_ave,map_std,l_rad,d_res)
    
    return pdb, alerts
# ad H as OH group to A3


def calc_EDIA(HO,alt,map,map_ave,map_std,l_rad,d_res):
    v_EDIA=0
    H_coord=atom_to_coord(HO)
    r_HO=ele_to_EDIA_rad(HO[14],l_rad,d_res)
#    print("r_HO",r_HO)
    r_suround=2*r_HO+2*2.16
#    print("r_suround",r_suround)
    l_atoms_s=find_sorunding_atoms(HO,pdb,d_res)
#    print(len(l_atoms_s))
#    print("l_atoms_s[0]",l_atoms_s[0])
#    for i in range(len(l_atoms_s)):
#        print(l_atoms_s[i])
    v_EDIA, inte_val=EDIA_value(HO, map, r_HO, mesh,l_atoms_s,map_ave,map_std)
#    print("v_EDIA",v_EDIA,"%%%%%%%%%%%%%%%%%%%%%%%%")
...
def EDIA_value(atom, map, r, mesh,l_atoms_s,map_ave,map_std,):
#integrates in certan radius around point
        r2=r*2
        int_val=0
        l_val=[]
        ...
        l_w_m=[]
        mesh = mesh *3
        atom_coord=atom_to_coord(atom)
        ran_int=range_to_int(atom_coord,r2,mesh)
        for i in range(ran_int[0],ran_int[1]):
          for j in range(ran_int[2],ran_int[3]):
            for k in range(ran_int[4],ran_int[5]):
                p=[i*mesh, j*mesh,k*mesh]
                dist=CDist2(atom_coord,p)
                if dist <= r2:
                  val=map.grid.interpolate_value(gemmi.Position(p[0],p[1],p[2]))
                  
                  w=calc_w(p,atom_coord,r)
                  z=calc_z(map_ave,map_std,val)
 #                 print("###P",p)
                  o = calc_o( p,atom,r, l_atoms_s)
                  woz=w*o*z
                  l_woz.append( woz)
                  l_w.append( w)
                  if w > 0:
                  ...
                  l_val.append(val)
        int_val=sum(l_val)#/vol
#        print("len l_woz",len(l_woz))
        EDIA=sum(l_woz)/sum(l_w_p)
        EDIA_p=sum(l_woz_p)/sum(l_w_p)
        EDIA_m=sum(l_woz_m)/sum(l_w_m)
        print(" ")
        print("woz,w, normal, p, m",round(sum(l_woz),2),round(sum(l_w_p),2),round(sum(l_woz_p),2),round(sum(l_w_p),2),round(sum(l_woz_m),2),round(sum(l_w_m),2))
        print("EDIA , p , m", round(EDIA,2), round(EDIA_p,2), round(EDIA_m,2))
        return EDIA,int_val
...
def calc_o(P1,atom,r, l_atoms_s):
#calculates o (ownershifp) for EDIA
    o=0
    s=[]
    p=[]
    sp=False
    dp=False 
    a_coord=atom_to_coord(atom)
#    print("p",P1)
#    print("a_coord",a_coord)
    dist=dist = CDist2(P1, a_coord )
    ...
    for i in range(len(l_atoms_s)):
#        print(len(l_atoms_s))
        P2=atom_to_coord(l_atoms_s[i])
#        print(l_atoms_s[i])
#        print("len(l_atoms_s[i][17])",len(l_atoms_s[i]))
        if l_atoms_s[i][14]=="D" or l_atoms_s[i][14]=="H":
            t=17
        else:
            t=16
        t = len(l_atoms_s[i])-1 # this is a hack
        r2=l_atoms_s[i][t][0]
        d2=CDist2(P1,P2)
        if d2 <= r2:
            if l_atoms_s[i][t][1]==False:
               s.append([d2,l_atoms_s[i]])
        if r2 < d2 < r2*2:
            if l_atoms_s[i][t][1]==False:
              p.append([d2,l_atoms_s[i]]) 
...
def calc_z(map_ave,map_std,val):
#calculate z(p) for EDIA
    zeta=1.2
    sig=(val-map_ave)/map_std
    if sig < 0:
        z=0
    if 0<= sig <= zeta:
        z=sig
...
def calc_w(p,atom,r):
#calculate w for EDIA
    x = CDist2(p,atom)
    P1=-1*(x)**2+1
    P2=5.1177*(x-1.29366*r)**2-0.4
    P3=-0.9507*(x-2*r)**2
    w=P1+P2+P3
    return w
...
def find_sorunding_atoms(HO,pdb,d_res):
    #find sourinding atoms with minimum distance r_suround
    P1 = atom_to_coord(HO)
    r_cov1=ele_to_cov_rad(HO[14])
    r_HO=ele_to_EDIA_rad(HO[14],l_rad,d_res)
    atoms=[]
    for i in range(len(pdb)):
        for j in range(len(pdb[i])):
            r_2=ele_to_EDIA_rad(pdb[i][j][14],l_rad,d_res)
            P2 = atom_to_coord(pdb[i][j])
            dist = CDist2(P1, P2)
            max_dist=2*r_HO+2*r_2
            if 0.5 < dist <= max_dist:
                r_cov2=ele_to_cov_rad(pdb[i][j][14])
                bond_dist=r_cov1+ r_cov2+0.4
                if dist <= bond_dist:
                    add=pdb[i][j].copy()
                    add.append([r_2,True])
                    atoms.append(add)
...
def ele_to_EDIA_rad(ele,l_rad,d_res):
#    find radius for elment and resolution
     if ele =="D":
         ele="H"
...
def    interpol_line(p1,p2,x):
#    interpolate beween two points an get valye for x
...
#find resolution from headder
    ...
    for i in range(len(head)):
      ...
      if len(line)>=4:
#        print(line)
        if line[2]=="NEUTRON" and line[3]=="DATA.":
           neutron =True
        if line[2]=="X-RAY" and line[3]=="DATA.":
           neutron =False
        if neutron ==True:
          if len(line)>=8:
            if line[2]=="RESOLUTION" and line[4]=="HIGH":
              res=line[7]
...
def find_bond_H(pdb,res_num,alt,A3):
##counts number of bonds for atom
#       cov = cov_rad()
        print("alt",alt)
        atom1, ele1 =anam_to_coord_ele(pdb,res_num,alt,A3)
...
def update_OH_EDIA1(pdb, res_num,alter,A1,A2,A3, map , alerts):
# ad H as OH group to A3
#     print("res name and number",pdb[res_num][0][4],pdb[res_num][0][6])
     av_CH,sig_CH= int_CH(pdb,res_num,alter,map,mesh)
     r_int=def_r_int("H")
...
org_pdb, pdb_read, head = read_pdb(input_pdb)

pdb = atm_to_res(pdb_read)
if map_ex==True:
        map,map_std, map_ave= read_grid(input_grid)
        map.setup()

        


pdb, alerts = treat_res(pdb, map) #used in setup


if EDIA_ex == True:
    l_rad=read_rad(input_rad)
    print(l_rad[1].r05)
#    pdb, alerts = treat_res_EDIA(pdb,map,l_rad)
    EDIA_CA=clac_EDIA_CA(pdb,map,l_rad)
...
#if len(clash_cov)>=1:
#    pdb = rm_cov_clash(pdb,clash_cov,map)
#    clash_cov,clash_vdw=find_clash(pdb)
...
#pdb=del_d_hbonds(pdb)

#pdb=del_d_hbonds_new(pdb,not_del_l,pka_ex) #used in setup
...
write_pdb(pdb,head,out_file)
time_ende = time.time()
print("program ends normally after ", time.strftime('%H:%M:%S', time.gmtime(time_ende-time_start)), "hh:mm:ss")
